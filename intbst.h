...
class IntBST {
 ...
 public:
    ...
    //leetcode
    int maxDepth() const; //max depth helper
    void sortedArrayToBST(vector<int>& nums); //build the bst
    int getkthsmallest(int k) const; //public
    int countLeaves() const; //number of leaves
    int countLeavesPreOrder() const; //pre order method
    vector<int> getKeysAtDepth(int d) const;
    int kthLargest(int k) const; //public    
    ...
    //some extra leetcode practice problems
    int maxDepth(const Node* root) const; //not modifying root nor member variables
    Node* buildHelper(vector<int>& nums, int left, int right, Node* p); //helper for creating the nodes and returning them based on the index of array
    Node* lca(Node* root, Node* p, Node* q); //lca 
    int getkthsmallesthelper(Node* n, int& count, int k_idx, int& k_val) const; //kth smallest
    void inOrderHelperLeaves(Node* n, int& c) const;
    int preOrderHelperLeaves(Node* n) const;
    void bfsHelper(Node* n, vector<int>& v, int targetDepth, int currentDepth) const; //helper function for in order  
    int kthLargestHelper(Node* n, int k_idx, int& count, int& k_val) const; //large helper
    int inOrderHelperCount(Node* n, int& c, int min, int max) const; // 
